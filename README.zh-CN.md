...
- **混合检索** — 语义与关键词双路检索融合，支持知识库级重排配置；内置检索测试，关键词 ↔ 语义权重可调。
- **多租户** — MySQL、Milvus、MinIO 全链路逻辑隔离。
- **文档流水线** — 上传（拖拽、粘贴、多文件、URL 导入）→ 解析（MinerU / 本地解析器）→ 分块（段落、Token、标题感知、问答对、父子分块）→ 向量化 → 索引，全程异步。
- **流式对话** — 实时流式输出，带来源引用、追问建议与多端流同步。客户端断开也不会丢失回答：即使关闭页面，回复仍在后端继续生成，完整回答（含计费）照常保存。DeepSeek 风格问候：居中空状态 + Agent 开场白 + 按日期分组的对话列表。
...
- **标注问答** — 为知识库维护人工标注的标准问答，命中即直接返回标准答案，保证高频问题的回答稳定一致。
- **自动记忆** — 长对话在后台自动生成摘要，摘要注入后续会话，实现跨对话上下文延续，支持站点级开关，并提供知识库级记忆页查看与删除摘要。
- **个人中心** — 聊天页点击左下角账号卡片菜单中的头像打开个人中心：基本信息、修改密码与「我的用量」（个人消息额度、Token 消耗、费用估算与最近调用记录），成员无需进入后台即可掌握自己的使用情况。
- **Agent 画布** — 可视化编排 Agent 流水线：意图分类 → 检索 → 条件分支 → LLM → 直接回复。节点输出以变量（`{node.output}`）在提示词与条件中引用，支持 LLM 链式调用与通用条件分支；单节点调试直接查看输出变量，运行时节点实时高亮。画布内置撤销/重做与便签。支持知识库级系统提示词、温度、检索参数、重排开关、开场白与 LLM 选择。
- **执行回放** — 每次 Agent 运行（真实对话与测试抽屉运行）都会记录逐节点轨迹，可在只读画布上回放；对话消息与数据分析反馈均可深链进入回放，用于调试与调优。
- **数据处理流水线画布** — 可视化入库流水线：来源 → 解析器 → 分块器 → 向量化 → 存储，可配置解析选项、分块策略与批大小。
- **知识库备份与恢复** — 将知识库（元数据、文档、分块）导出为 JSON 文件，可导入到任意知识库；导入后自动重建索引以恢复向量。
- **GraphRAG** — 入库时 LLM 抽取实体与关系，查询时实体匹配增强检索上下文。
- **模型可配置** — 租户级管理 LLM / Embedding / Rerank 模型，知识库可单独指定。双栏弹窗选择器（左侧供应商列表、右侧模型列表），切换直观高效。每个模型行支持折叠式容量设置（最大输出 Token、计费单价，元/百万 Token），未保存的草稿模型也可先测试再保存。选择优先级：知识库配置 → 租户默认。
