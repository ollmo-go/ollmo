...
*知识问答 —— 流式回答附带来源引用，点击引用即打开源文档并高亮命中段落，对话与文档并排对照*
...
- **文档流水线** — 上传（拖拽、粘贴、多文件、URL 导入）→ 解析（MinerU / 本地解析器）→ 分块（段落、Token、标题感知、问答对、父子分块）→ 向量化 → 索引，全程异步（Asynq + Redis）。
- **流式对话** — SSE 实时流式输出，带来源引用、追问建议与多端流同步。客户端断开也不会丢失回答：LLM 流与请求解耦、在后端继续运行，即使关闭页面，完整回答（含计费）照常落库。DeepSeek 风格问候：居中空状态 + Agent 开场白 + 按日期分组的对话列表。
- **引用溯源** — 点击回答下方的引用即打开源文档并高亮命中段落：PDF 原版渲染、解析视图翻页导航、对话 + 文档并排对照，「答案有据可查」落到原文。
- **回答反馈** — 对任意回答可点赞/点踩；点踩进入数据分析的「坏例复盘」列表，并可直接深链跳转到对应执行回放进行调试。
- **标注问答** — 为知识库维护人工标注的标准问答（向量相似度匹配），命中即直接返回标准答案，保证高频问题的回答稳定一致。
- **自动记忆** — 长对话在后台自动生成摘要（异步 LLM 任务），摘要注入后续会话，实现跨对话上下文延续，支持站点级开关，并提供知识库级记忆页查看与删除摘要。
- **个人中心** — 聊天页点击左下角账号卡片菜单中的头像打开个人中心：基本信息、修改密码与「我的用量」（个人消息额度、Token 消耗、费用估算与最近调用记录），成员无需进入后台即可掌握自己的使用情况。
...
- **执行回放** — 每次 Agent 运行（真实对话与测试抽屉运行）都会记录逐节点轨迹，可在只读画布上回放；对话消息与数据分析反馈均可深链进入回放，用于调试与调优。
- **数据处理流水线画布** — 可视化入库 DAG（React Flow）：来源 → 解析器 → 分块器 → 向量化 → 存储，可配置解析选项、分块策略与批大小。
- **知识库备份与恢复** — 将知识库（元数据、文档、分块）导出为 JSON 文件，可导入到任意知识库；导入后自动重建索引以恢复向量。
- **GraphRAG** — 入库时 LLM 抽取实体与关系，查询时实体匹配增强检索上下文。
- **模型可配置** — 租户级管理 LLM / Embedding / Rerank 模型，知识库可单独指定。双栏弹窗选择器（左侧供应商列表、右侧模型列表），切换直观高效。每个模型行支持折叠式容量设置（最大输出 Token、计费单价，元/百万 Token），未保存的草稿模型也可先测试再保存。选择优先级：知识库配置 → 租户默认 → config.yaml。
...
- **管理控制台** — 超级管理员用户管理、团队管理（计划与配额：免费版 / 专业版 / 企业版）、系统设置（注册开关、自动记忆、自定义站点 Logo）、数据分析看板、审计日志。
...
│   │   ├── agent/          # Agent 画布配置与执行
│   │   ├── analytics/      # 用量统计与动态
│   │   ├── annotation/     # 标注问答对（人工维护的标准答案）
│   │   ├── apikey/         # API Key 管理与中间件
│   │   ├── audit/          # 审计日志
│   │   ├── auth/           # 认证（邮箱 + 密码 + JWT）
│   │   ├── backup/         # 知识库导出/导入（JSON 备份与恢复）
│   │   ├── bill/           # 用量与费用（Token / 成本记账）
│   │   ├── chat/           # 流式对话与引用
│   │   ├── doc/            # 文档生命周期、分块、worker
│   │   ├── embedding/      # 嵌入模型管理
│   │   ├── execution/      # Agent 运行轨迹与回放
...
│   │   ├── provider/       # 统一模型供应商卡片（LLM / Embedding / Rerank）
│   │   ├── rerank/         # 重排模型管理
...
| Web UI → 系统设置 | 站点级设置（注册开关、自动记忆、自定义 Logo）— 仅超级管理员 |
...
- [x] **阶段 4 — 生产就绪**：数据分析看板、用量与费用（账单）、审计日志、对话管理（搜索 / 重命名 / 导出 / 置顶）、自动记忆摘要、个人中心、标注问答、超级管理员控制台（用户、团队、计划与配额、系统设置）、注册开关、知识库备份与恢复、执行回放、回答反馈。
- [ ] **阶段 5 — 文档智能**：

  ### 5.1 文档预览与引用溯源 ✅
  - [x] 对话中点击引用跳转至源文档，并高亮引用段落
  - [x] 文档查看器：PDF 渲染、Markdown/文本预览、翻页导航
  - [x] 分块到来源的映射（分块 → 文档 → 页码/偏移）
  - [x] 并排视图：对话 + 源文档
