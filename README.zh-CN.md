...
- **混合检索** — Milvus 稠密向量 + MySQL FULLTEXT 词法匹配，RRF 融合，支持知识库级 Rerank 配置。
...
│   └── config.yaml
├── web/                    # Next.js 前端
│   ├── app/                # App Router 页面（'/' 对话，'/dashboard/*' 管理后台）
...
- [x] **阶段 4 — 生产就绪**：数据分析看板、审计日志、对话管理（搜索 / 重命名 / 导出 / 置顶）、自动记忆摘要、超级管理员控制台（用户、团队、计划与配额、系统设置）、注册开关。
- [ ] **阶段 5 — 文档智能**：
