...
export default function RegisterPage() {
  ...
  // matching the backend check. Missing/other values disable it.
  const allowRegistration = settings?.allow_registration === "true";

  // Redirect to home if already logged in, or to login if registration is
  // disabled. Wait for settings before rendering the form so the form never
  // flashes when registration is off.
  useEffect(() => {
    if (isAuthenticated()) {
      router.replace("/");
  ...
  }, [settings, allowRegistration, router]);
  ...
  // Don't render the form until settings are loaded and registration is
  // confirmed enabled; otherwise the form flashes before the redirect.
  ...
  return (
    <div className="min-h-screen flex items-center justify-center">
      ...
      <div className="w-[420px]">
        ...
        <Card>
        <CardHeader>
          <CardTitle className="text-xl">{t("auth.register_title_short")}</CardTitle>
          <CardDescription>
        ...
        <form onSubmit={handleSubmit}>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="name">{t("auth.display_name")}</Label>
              <Input
                id="name"
                value={name}
          ...
          <CardFooter className="flex flex-col gap-3">
            <Button type="submit" className="w-full" disabled={loading}>
              {loading ? t("auth.creating") : t("auth.create_tenant")}
            </Button>
